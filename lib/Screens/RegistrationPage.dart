...
class K1ScreenState extends State<K1Screen> {
  ...
  void _signup() async {
    ...
    try {
      ...
      User? user = await _auth.signup(
        ...
        password: password,
        username: username,
        address: "null", // يمكن للمستخدم تعديل العنوان لاحقاً
        phone: phoneNumber,
        isDriver: isDriver,
      ...
      if (user != null) {
        ...
        showToast(message: "حدث خطأ :رقم الهاتف مسجل مسبقاً"); 
      }
    } catch (e) {
  ...
  Future<void> _verifyPhoneNumber(String phoneNumber) async {
    ...
    if (!RegExp(r'^\+\d{1,3}\d{1,14}$').hasMatch(phoneNumber)) {
      showToast(
          message:
              "رقم الهاتف غير صالح. يجب أن يبدأ برمز الدولة (+966) متبوعًا برقم الهاتف.");
